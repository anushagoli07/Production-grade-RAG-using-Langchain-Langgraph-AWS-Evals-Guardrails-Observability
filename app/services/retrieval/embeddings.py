...
def __probe_gemini():
    try:
        ...
        logfire.info("Gemini embeddings are ready (gemini-embedding-001, 3072-dim)")
        return model
...
def _embed_batch(batch: list[str]) -> list[list[float]]:
    if _model_type == "gemini":
        ...
        for attempt in range(4):
            try:
                result = _active_model.embed_documents(batch)
                time.sleep(1.0)  # proactive throttle — stay under RPM even on success
                return result
            except Exception as e:
                err = str(e).lower()
